...
export const addUser = async(user) => {
    ...
    try{
        const response = await axios.post(`http://localhost:5000/GameServer/users`,
...
export const getRanking = async() => {
    try{
        const response = await axios.get(`http://localhost:5000/GameServer/users/ranking/status/all`);
        return response
    }
    catch(error){
        console.log(error)
...
export const getUserById = async(id) => { 
        ...
        try {
            return await axios.get(`http://localhost:5000/GameServer/users/${id}`);    
        } catch (error) {
