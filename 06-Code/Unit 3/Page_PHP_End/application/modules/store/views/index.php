...
<div class="Pane-bg" style="background-color:#041828;background-image:url(&quot;<?= $template['assets'].'core/css/images/de9g3fg-6bef3bf6-e4cd-4c6d-bb01-96dfd7b680f4.jpg'; ?>&quot;);">
...
<div class="gutter-huge">
        <div class="uk-grid uk-grid-medium" data-uk-grid>
          <div class="uk-width-1-4@m">
            <div class="uk-card uk-card-default" style="background:#0356b980">
              ...
              <ul class="uk-nav-default nav-store uk-nav-parent-icon" uk-nav>
                <li class="uk-active"><a href="<?= base_url('store'); ?>" style="font-size:16px"><i class="fas fa-star"></i> <?= $this->lang->line('store_top_items'); ?></a></li>
                <?php foreach ($this->wowrealm->getRealms()->result() as $MultiRealm): ?>
                <li class="uk-parent">
                  <a href="javascript:void(0);" style="font-size:16px"><i class="fas fa-server"></i> <?= $this->wowrealm->getRealmName($MultiRealm->realmID); ?></a>
                  <ul class="uk-nav-sub uk-nav-parent-icon" uk-nav>
                    ...
                    <li class="uk-parent">
                      <a href="#" style="font-size:16px"><?= $menulist->name ?></a>
                      <ul class="uk-nav-sub">
                        <?php foreach ($this->store_model->getChildStoreCategory($menulist->id)->result() as $menuchildlist): ?>
                        <li><a href="<?= base_url('store/'.$menuchildlist->route) ?>" style="font-size:16px"><?= $menuchildlist->name ?></a></li>
                        <?php endforeach; ?>
                      </ul>
                    </li>
                    <?php elseif($menulist->main == '1' && $menulist->father == '0'): ?>
                    <li><a href="<?= base_url('store/'.$menulist->route) ?>" style="font-size:16px"><?= $menulist->name ?></a></li>
                    <?php endif; ?>
                    <?php endforeach; ?>
                  </ul>
                </li>
          ...
          <div class="uk-width-3-4@m">
            <div class="uk-card uk-card-default uk-card-body" style="background:#0356b980">
                  ...
                  <div class="blizzcms-item-container">
                    <div class="blizzcms-item-header uk-text-truncate" uk-tooltip="<?= $this->store_model->getName($top->store_item); ?>" uk-toggle="target: #top-<?= $top->id ?>;animation: uk-animation-slide-top-small">
                      ...
                      <span class="uk-text-middle">
                        ...
                        <a href="https://wowhead.com/item=<?= $this->store_model->getCommand($top->store_item); ?>" style="font-size:16px"><?= $this->store_model->getName($top->store_item); ?></a>
                        <?php else: ?>
                        <a href="#" style="font-size:16px"><?= $this->store_model->getName($top->store_item); ?></a>
                        <?php endif; ?>
                      </span>
                      <!-- END TOOLTIPS WOWHEAD -->
                    </div>
                    <div id="top-<?= $top->id ?>" class="blizzcms-item-body" hidden>
                      <p class="uk-text-break" style="font-size:16px"><?= $this->store_model->getDescription($top->store_item); ?></p>
                      ...
                      <div class="uk-grid uk-grid-small uk-flex uk-flex-center" data-uk-grid>
                        <div class="uk-width-auto">
                          ...
                          <span class="blizzcms-item-price" style="font-size:16px"><span uk-tooltip="title: <?= $this->lang->line('panel_dp'); ?>"><i class="dp-icon"></i></span><?= $this->store_model->getPriceDP($top->store_item); ?></span>
                          <?php elseif($this->store_model->getPriceType($top->store_item) == 2): ?>
                          <span class="blizzcms-item-price" style="font-size:16px"><span uk-tooltip="title: <?= $this->lang->line('panel_vp'); ?>"><i class="vp-icon"></i></span><?= $this->store_model->getPriceVP($top->store_item); ?></span>
                          <?php elseif($this->store_model->getPriceType($top->store_item) == 3): ?>
                          <span class="blizzcms-item-price" style="font-size:16px"><span uk-tooltip="title: <?= $this->lang->line('panel_dp'); ?>"><i class="dp-icon"></i></span><?= $this->store_model->getPriceDP($top->store_item); ?> <span class="uk-badge">&amp;</span> <span uk-tooltip="title: <?= $this->lang->line('panel_vp'); ?>"><i class="vp-icon"></i></span><?= $this->store_model->getPriceVP($top->store_item); ?></span>
                          <?php endif; ?>
                        </div>
                        <div class="uk-width-auto">
