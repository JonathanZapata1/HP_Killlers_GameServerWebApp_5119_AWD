...
export const getRanking = async() => {
    try{
        const response = await axios.get(`http://54.210.206.181:3005/GameServer/users/ranking/status/all`);
        return response
    }
    catch(error){
        console.log(error)
...
export const getUserById = async(id) => { 
        ...
        try {
            return await axios.get(`http://54.210.206.181:3005/GameServer/users/${id}`);    
        } catch (error) {
